...
def horizon_concate(inp0, inp1):
    ...
    h1, w1 = inp1.shape[:2]
    if inp0.ndim == 3:
        inp = np.zeros((max(h0, h1), w0 + w1, 3), dtype=np.uint8)
        inp[:h0, :w0, :] = inp0*255
        inp[:h1, w0:(w0 + w1), :] = inp1 * 255
    else:
        inp = np.zeros((max(h0, h1), w0 + w1), dtype=np.uint8)
        inp[:h0, :w0] = inp0*255
        inp[:h1, w0:(w0 + w1)] = inp1*255   #make the image range ok
    return inp
